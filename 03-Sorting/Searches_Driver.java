	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			if (found == -1)
				...
				System.out.println("Linear Search found it at location " + found + " in " + Searches.getLinearCount()
						+ " comparisons.");
...
class Searches {
	...
	public static int binary(Comparable[] array, Comparable target) {
		if (array.length > 0) {
			binaryCount++;
			if (array[array.length / 2].compareTo(target) == 0) {
				return array.length / 2;
			} 
			else if (array[array.length / 2].compareTo(target) > 0) {
				return binaryhelper(array, target, 0, array.length/2 - 1);
			} 
			else if (array[array.length / 2].compareTo(target) < 0) {
				return binaryhelper(array, target, array.length/2 + 1, array.length);
			}
		}
		return -1;
	}
	...
	private static int binaryhelper(Comparable[] array, Comparable target, int start, int end) {
		...
		if (end - start >= 0) {
			...
			else if (array[(start + end) / 2].compareTo(target) > 0) {
