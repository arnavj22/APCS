...
class MergeSort {
	...
	/*
	 * array array that is being sorted copyBuffer temp space needed during the
	 * merge process low, high bounds of subarray middle midpoint of subarray
	 */
	...
	/*
	 * array array that is being sorted copyBuffer temp space needed during the
	 * merge process low beginning of first sorted subarray middle end of first
	 * sorted subarray middle + 1 beginning of second sorted subarray high end of
	 * second sorted subarray
	 */
	public static void merge(double[] array, double[] copyBuffer, int low, int middle, int high) {
		...
		while (low <= middle && j <= high) {
			double i1 = copyBuffer[low];
			// System.out.println(high +" " + middle +" "+low);
			double i2 = copyBuffer[j];
			if (i1 <= i2) {
				array[k] = i1;
				low++;
			} 
	...
	public static void merge(Comparable[] array, Comparable[] copyBuffer, int low, int middle, int high) {
		...
		int j = middle + 1;
		while (low <= middle && j <= high) {
			Comparable i1 = copyBuffer[low];
			// System.out.println(high +" " + middle +" "+low);
			Comparable i2 = copyBuffer[j];
